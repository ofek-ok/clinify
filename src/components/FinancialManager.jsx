...
const FinancialManager = () => {
  ...
  const exportFinancialReportCsv = () => {
    ...
    csvContent += "סוג,תאריך,שם,קטגוריה/שירות,סכום (₪),אמצעי תשלום,סטטוס\n";

    payments.forEach(p => {
      const patientName = getPatientName(p.patient_id) || 'ללא שם';
      const itemName = getServiceName(p.catalog_item_id) || 'טיפול/שירות';
      const dateStr = p.payment_date ? p.payment_date.split('T')[0] : '';
      csvContent += `הכנסה,${dateStr},"${patientName}","${itemName}",${p.amount},"${p.payment_method}","${p.status}"\n`;
    });

    expenses.forEach(e => {
      const dateStr = e.expense_date ? e.expense_date.split('T')[0] : '';
      csvContent += `הוצאה,${dateStr},"${e.title}","${e.category}",-${e.amount},"${e.payment_method}","שולם"\n`;
    });

    const encodedUri = encodeURI(csvContent);
    const link = document.createElement("a");
    link.setAttribute("href", encodedUri);
    link.setAttribute("download", `clinify_financial_report_${new Date().toISOString().split('T')[0]}.csv`);
