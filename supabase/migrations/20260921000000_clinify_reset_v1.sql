-- ========================================================
-- Clinify Product Reset V1 — Real Supabase Migration
-- Target Project: calify (stwgtsmdtjfwfkibzdlh)
-- Incremental schema additions & canonical identity fixes
-- ========================================================

-- 1. Canonical Identity Fixes: Make phone & normalized_phone nullable on people table
ALTER TABLE people ALTER COLUMN phone DROP NOT NULL;
ALTER TABLE people ALTER COLUMN normalized_phone DROP NOT NULL;
...
-- 4. Ensure Content Items Table Exists
...
-- 6. RLS Policies (Authenticated Access Only — Anon has ZERO access to projects & content_items)
...
-- 7. Clean Public Pre-Launch Signup RPC (Business Transaction Only: Person -> Lead)
...
BEGIN
  ...
  -- Normalize Phone & Email (Use NULL instead of empty string for phone)
  ...
  -- 2. Reuse or Create Lead record (only for non-customers)
