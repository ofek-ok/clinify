  ...
  return (
    <div className="animate-in fade-in duration-500 space-y-6 text-start">
      <div className="mb-6 flex justify-between items-end">
        <div>
          <h2 className="text-2xl font-bold text-slate-800">{t('Patient Directory', 'ניהול מטופלים')}</h2>
          <p className="text-slate-500 text-sm mt-1">{t('View, search, and add new patients to the system.', 'צפה, חפש והוסף מטופלים חדשים למערכת.')}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-1">
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100/50 relative overflow-hidden h-max">
            <div className="absolute top-0 end-0 w-full h-1 bg-gradient-to-s from-blue-400 to-indigo-500"></div>
            ...
            <form onSubmit={handlePatientSubmit} className="space-y-4">
              <div>
                ...
                <input type="text" value={patientForm.full_name} onChange={e => setPatientForm({...patientForm, full_name: e.target.value})} required 
                  className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all text-start" />
              </div>
              <div>
              ...
              <div>
                ...
                <input type="tel" value={patientForm.phone} onChange={e => setPatientForm({...patientForm, phone: e.target.value})} required 
                  className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all text-end" dir="ltr" />
              </div>
              <button type="submit" className="w-full mt-2 bg-gradient-to-e from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700 text-white font-medium py-2.5 px-4 rounded-xl transition-all shadow-md active:scale-[0.98]">
            ...
            <div className="p-4 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
              <div className="relative w-64">
                <input type="text" placeholder={t('Search patients...', 'חיפוש מטופלים...')} value={searchTerm} onChange={e => setSearchTerm(e.target.value)} 
                  className="w-full ps-4 pe-10 py-2 bg-white border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-500/20 outline-none text-start" />
                ...
                <tbody className="divide-y divide-slate-50">
                  {filteredPatients.map(patient => (
                    <tr key={patient.id} className="hover:bg-slate-50/50 transition-colors">
                      <td className="py-3.5 px-6 text-sm font-semibold text-slate-700 text-start">{patient.full_name}</td>
                  ...
                  {filteredPatients.length === 0 && (
                    <tr>
                      <td colSpan="4" className="py-8 text-center text-slate-400 text-sm">{t('No matching patients found.', 'לא נמצאו מטופלים תואמים.')}</td>
                    </tr>
                  )}
                </tbody>
              </table>
