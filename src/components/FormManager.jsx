...
export default function FormManager({ navigate }) {
  ...
  const copyToClipboard = async (formId) => {
    try {
      ...
      showToast('לא ניתן להעתיק את הקישור', 'error');
    }
  };
  ...
  const submissionDisplayName = (form, submission) => {
    ...
    return firstText ? String(firstText[1]).slice(0,45) : 'מענה ללא שם';
  };
    ...
    <div className="space-y-4 dir-rtl text-start font-sans">
      <div className="premium-panel flex flex-wrap items-center justify-between gap-3 p-4 rounded-2xl">
        <div>
          ...
          <p className="mt-1 text-xs text-slate-500">Intake, הסכמות, שאלונים וטפסי מעקב.</p>
        </div>
          ...
          <table className="w-full min-w-[760px] text-start border-collapse text-xs">
            <thead>
              <tr className="bg-slate-50/80 border-b border-slate-200 text-[11px] font-bold text-slate-500">
                ...
                <th className="py-3 px-4 text-start">גישה</th>
                <th className="py-3 px-4 text-start">תשובות</th>
                <th className="py-3 px-4 text-start">נוצר</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {forms.length === 0 ? (
                <tr><td colSpan={5} className="py-14 text-center text-slate-500">
                  אין עדיין טפסים. <button onClick={() => navigate('formBuilder')} className="text-violet-600 font-bold">צור את הראשון</button>
                </td></tr>
              ) : forms.map(form => {
                  ...
                  <tr key={form.id} className="hover:bg-slate-50/80 transition-colors">
                      ...
                      <button type="button"
                        onClick={async () => {
                          try {
                            ...
                            showToast(form.is_public === true ? 'הטופס הוגדר כפנימי' : 'הטופס הוגדר כציבורי');
                          } catch(err) { showToast(err.message || 'לא ניתן לעדכן', 'error'); }
                        }}
                        ...
                        {form.is_public === true ? 'ציבורי' : 'פנימי'}
                      </button>
                    </td>
                    <td className="py-3.5 px-4">
                    ...
                    <td className="py-3.5 px-4">
                      <div className="flex items-center justify-end gap-1">
                        <button onClick={() => editForm(form)} title="עריכה" className="p-1.5 rounded-lg text-slate-400 hover:text-violet-600 hover:bg-violet-50"><Pencil className="w-4 h-4"/></button>
                        {form.is_public === true && <>
                          <button onClick={() => copyToClipboard(form.id)} title="העתק קישור" className="p-1.5 rounded-lg text-slate-400 hover:text-slate-900 hover:bg-slate-100"><Copy className="w-4 h-4"/></button>
                          <a href={`/form/${form.id}`} target="_blank" rel="noreferrer" title="תצוגה ציבורית" className="p-1.5 rounded-lg text-slate-400 hover:text-violet-600 hover:bg-violet-50"><ExternalLink className="w-4 h-4"/></a>
                        </>}
                        <button type="button" onClick={async()=>{
                          if(!window.confirm(`להעביר את הטופס "${form.title}" לאשפה?`)) return;
                          try { await softDeleteRecord('forms',form.id); showToast('הטופס הועבר לאשפה'); }
                          catch(err){ showToast(err.message || 'לא ניתן להעביר לאשפה','error'); }
                        }} title="העבר לאשפה" className="p-1.5 rounded-lg text-slate-400 hover:text-rose-600 hover:bg-rose-50"><Trash2 className="w-4 h-4"/></button>
                      </div>
                    </td>
                  </tr>
                );
      ...
      <Drawer isOpen={Boolean(selectedForm)} onClose={closeDrawer}
        title={selectedSubmission ? 'פרטי מענה' : selectedForm?.title || 'תשובות'}
        width="max-w-2xl"
        footer={<button type="button" onClick={closeDrawer} className="px-4 py-2 rounded-xl bg-slate-950 text-white text-xs font-bold">סגור</button>}>
        {selectedForm && !selectedSubmission && (
          <div className="space-y-3">
            <div className="rounded-xl bg-slate-50 border border-slate-200 p-3 text-xs text-slate-500">
              {selectedSubmissions.length} תשובות התקבלו
            </div>
            {selectedSubmissions.length===0 ? (
              <div className="py-12 text-center text-xs text-slate-400">עדיין אין תשובות לטופס הזה.</div>
              ...
              <button key={submission.id} type="button" onClick={()=>setSelectedSubmission(submission)}
                ...
                <div className="min-w-0">
                  ...
                  <div className="mt-1 text-[10px] text-slate-400">{new Date(submission.submitted_at).toLocaleString('he-IL')}</div>
                </div>
        ...
        {selectedForm && selectedSubmission && (
          <div className="space-y-5">
            <button type="button" onClick={()=>setSelectedSubmission(null)} className="text-xs font-bold text-violet-700">← חזרה לכל התשובות</button>

            <div>
              <label className="block text-[10px] font-bold text-slate-500 mb-1">שיוך ללקוח / ליד</label>
              <select value={selectedSubmission.person_id || ''} onChange={async e=>{
                try {
                  ...
                  showToast('השייכות עודכנה');
                } catch(err){ showToast(err.message || 'לא ניתן לשייך','error'); }
            ...
            <button type="button" onClick={async()=>{
              if(!window.confirm('להעביר את המענה לאשפה?')) return;
              try {
                await deleteFormSubmission(selectedSubmission.id);
                showToast('המענה הועבר לאשפה');
                setSelectedSubmission(null);
              } catch(err){ showToast(err.message || 'לא ניתן למחוק את המענה','error'); }
              ...
              <Trash2 className="w-4 h-4" /> העבר מענה לאשפה
            </button>
          </div>
        )}
      </Drawer>
